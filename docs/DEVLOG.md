...
- **看板布局与交互迭代**（`489e325`，响应"布局不太行/形态只有箭头/滑窗默认区间与记忆"反馈）：
  - **布局重排**：结论横条置顶（标的 | 机会/风险大字 | 建议+仓位 | 状态 | 明日区间摘要），第一屏即全部关键信息；右栏只留细节（预判/形态/研究视角/报告折叠）；触发明细移图下方。
  - **形态文字指示**：最近 10 个形态名直接标注在 K 线箭头旁（描边防遮挡），更早的箭头+悬停。
  - **滑窗**：默认最近 30 根；用户调整后 `localStorage`（key `meridian_zoom`）记忆起止百分比，跨标的/重启保持。
  - 验证方式沉淀：browser-use 真实浏览器截图 + evaluate 抓页面状态（err/canvas/topbar/localStorage），dispatchAction 模拟拖动验证记忆写入——**前端改动一律真实浏览器过一遍再交付**。

- **Mimosa 完整深度安全扫描（首次跑通）**：scanId `scan-2026-09-05T13-35-37.780Z-3061b128b0a4`，seal `sha256:8df761d2...e9d5`，产物在 `~/.mimosa/security-scans/project-597d.../`。**代码层 findings 0 条**；依赖 536 包扫描完成，离线漏洞库匹配 2 包/2 公告（明细在产物目录）。此前每次 commit 的"扫描未完整"警告至此补上正式审计。不宣称"项目绝对安全"——仅本次静态扫描结论。
- **K线形态识别 + 图表量化指标升级**（响应"图里没有量化指示/K线形态指示"反馈）：
  - `patterns.py`：9 类经典形态纯 pandas 识别（十字星/光头光脚/锤子/流星/阳阴吞没/启明星/黄昏之星/红三兵/三只乌鸦），几何判定+含义说明，**不构成信号**；报告形态表（最近10）+ API patterns 字段 + 看板图上三角标注（红▲偏多在低点/绿▼偏空在高点/灰圈中性）+ 右侧最近 5 条列表。茅台 162 根识别 77 处，最新 09-04 启明星（bullish）。
  - 看板图新增 **MACD 副图**（DIF/DEA/柱）与 **RSI14 副图**（30/70 参考线），布局五区（价格/量/MACD/RSI/信号色带），dataZoom 全联动；前端 JS 自算展示（EMA/Wilder RSI 口径与通用 TA 教材一致）。
  - 教训：形态识别阈值测试数据易踩浮点边界（us=0.1500000000005 vs 0.5×b=0.1499...）——测试构造留余量，不做等值边界断言。
